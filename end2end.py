...
class End2End:
    ...
    def _train_model(self, device, model, train_loader, criterion_seg, criterion_dec, optimizer, validation_set, tensorboard_writer):
        ...
        for epoch in range(num_epochs):
            ...
            if self.cfg.VALIDATE and (epoch % validation_step == 0 or epoch == num_epochs - 1):
                validation_f1, validation_ap, validation_accuracy = self.eval_model(device, model, validation_set, None, False, True, False)
                validation_data.append((validation_f1, epoch))

                # Save best model based on F1-Score (instead of AP)
                if validation_f1 > max_validation:
                    max_validation = validation_f1
                    self._save_model(model, "baseline_best.pth")
                    self._log(f"Saved best model with F1={validation_f1:.4f} at epoch {epoch+1}")
    ...
    def eval_model(self, device, model, eval_loader, save_folder, save_images, is_validation, plot_seg):
        ...
        if is_validation:
            ...
            # Return F1-Score (best_f_measure), AP, and accuracy
            return metrics["best_f_measure"], metrics["AP"], metrics["accuracy"]
        else:
    ...
    def _save_model(self, model, name="final_state_dict.pth"):
        ...
        # Also save baseline_best.pth to root directory for convenience
        if name == "baseline_best.pth":
            root_output = os.path.join(os.path.dirname(os.path.abspath(__file__)), "baseline_best.pth")
            torch.save(model.state_dict(), root_output)
            self._log(f"Also saved baseline_best.pth to root directory: {root_output}")
