"""
C3: L1 + Activation Correlation Pruning for SegDecNet
An incremental improvement over Naive L1 pruning by detecting and removing redundant channels

Core Idea:
1. Compute L1 importance scores (same as Naive L1)
2. Collect activation patterns during forward pass
3. Calculate correlation matrix between channels
4. Penalize redundant channels (high correlation = high redundancy)
5. Prune based on corrected importance scores
"""
...
class PrunedSegDecNet(nn.Module):
    ...
    def __init__(self, device, input_width, input_height, input_channels, channel_config):
        ...
        if input_width % 8 != 0 or input_height % 8 != 0:
            raise Exception("Input size must be divisible by 8!")
...
def compute_activation_correlation(model, data_loader, device, num_batches=5):
    """
    Compute activation correlation for each conv layer

    For each layer, we:
    1. Collect activation statistics during forward pass
    2. Compute correlation matrix between channels
    3. Identify redundant channels (high correlation with other channels)

    Returns: redundancy_scores dict for each layer
    """
    activations = {}
    hooks = []

    # Register forward hooks to capture activations
    # Use named_modules() to access the original model's layers
    def get_activation(name):
        def hook(module, input, output):
            # output shape: [B, C, H, W]
            # We compute mean activation over spatial dimensions for each channel
            act = output.detach()
            # Mean over batch and spatial dims -> [C]
            channel_act = act.mean(dim=(0, 2, 3))
            if name not in activations:
                activations[name] = []
            activations[name].append(channel_act.cpu())

        return hook

    # Map state_dict keys to actual modules in the original SegDecNet
    # The model.volume is an nn.Sequential, so we access by index
    volume_layers = {
        "volume.0.0": (model.volume, 0, 0),  # volume[0][0] = Conv2d
        "volume.2.0": (model.volume, 2, 0),  # volume[2][0] = Conv2d
        "volume.3.0": (model.volume, 3, 0),  # volume[3][0] = Conv2d
        "volume.4.0": (model.volume, 4, 0),  # volume[4][0] = Conv2d
        "volume.6.0": (model.volume, 6, 0),  # volume[6][0] = Conv2d
        "volume.7.0": (model.volume, 7, 0),  # volume[7][0] = Conv2d
        "volume.8.0": (model.volume, 8, 0),  # volume[8][0] = Conv2d
        "volume.9.0": (model.volume, 9, 0),  # volume[9][0] = Conv2d
        "volume.11.0": (model.volume, 11, 0),  # volume[11][0] = Conv2d
    }

    for name, (parent_module, idx, subidx) in volume_layers.items():
        conv_module = parent_module[idx][subidx]
        hooks.append(conv_module.register_forward_hook(get_activation(name)))
    ...
    # Compute redundancy scores based on activation correlation
    redundancy_scores = {}
    ...
    for name, act_list in activations.items():
        ...
        # Stack all activations: [total_batches, C]
        all_acts = torch.stack(act_list)  # [num_batches, C]

        # Average over batches -> [C]
        avg_acts = all_acts.mean(dim=0)
        C = avg_acts.shape[0]

        # Only process layers with sufficient channels
        if C < 10:
            continue

        # Use different thresholds based on layer size
        # Larger layers need higher thresholds to avoid over-penalizing
        if C >= 512:
            corr_threshold = 0.98  # Very high threshold for large layers
        elif C >= 128:
            corr_threshold = 0.95
        else:
            corr_threshold = 0.90

        # Compute pairwise correlation using cosine similarity
        # Normalize each channel's activation vector
        centered = avg_acts - avg_acts.mean()
        norm = torch.norm(centered, p=2)
        if norm < 1e-6:
            continue
        normalized = centered / norm

        # Compute correlation matrix using matrix multiplication
        corr_matrix = torch.outer(normalized, normalized)

        # Find highly correlated channel pairs (corr > threshold)
        redundancy_score = torch.zeros(C)

        for i in range(C):
            for j in range(i + 1, C):
                if corr_matrix[i, j] > corr_threshold:
                    # Both channels are redundant with each other
                    corr_strength = corr_matrix[i, j].item()
                    redundancy_score[i] += corr_strength
                    redundancy_score[j] += corr_strength

        redundancy_scores[name] = redundancy_score.numpy()

        # Debug info
        n_redundant = (redundancy_score > 0).sum()
        if n_redundant > 0:
            print(f"    {name}: {n_redundant}/{C} channels have high correlation (thresh={corr_threshold})")

    return redundancy_scores


def compute_c3_importance(model, data_loader, device, corr_penalty_strength=0.3):
    """
    Compute C3 importance: L1 + Activation Correlation

    - Start with L1 importance (same as Naive L1)
    - Subtract redundancy penalty based on activation correlation

    Higher correlation with other channels -> more redundant -> lower final importance

    Note: We skip large layers (c5 with 1024 channels) from correlation penalty
    as their activation patterns are too diverse to find meaningful redundancy.
    """
    print("\n  Step 1: Computing L1 importance scores...")
    l1_scores = compute_l1_importance(model)

    print("\n  Step 2: Computing activation correlation redundancy...")
    redundancy_scores = compute_activation_correlation(model, data_loader, device, num_batches=10)

    # Combine L1 scores with redundancy penalty
    final_scores = {}
    print("\n  Step 3: Combining L1 with redundancy penalty...")

    for name, l1 in l1_scores.items():
        l1_tensor = torch.tensor(l1)

        if name in redundancy_scores and "volume.11.0" not in name:  # Skip c5 layer
            red = torch.tensor(redundancy_scores[name])

            # Normalize redundancy to [0, 1] scale relative to L1
            if red.max() > 0:
                red_normalized = red / (red.max() + 1e-6)
            else:
                red_normalized = red

            # Final score = L1 - penalty * normalized_redundancy
            # This reduces importance for redundant channels
            penalty = red_normalized * l1_tensor.max() * corr_penalty_strength
            final_score = (l1_tensor - penalty).numpy()

            n_penalized = (red_normalized > 0).sum()
            print(f"    {name}: L1 range [{l1.min():.2f}, {l1.max():.2f}], penalizing {n_penalized} channels")
        else:
            final_score = l1
            if "volume.11.0" in name:
                print("    {name}: Using pure L1 (skipped correlation for large layer)")
...
class FineTuner:
    ...
    def __init__(self, model, train_loader, val_loader, device, config):
        ...
        self.optimizer = torch.optim.SGD(
            model.parameters(), lr=config["learning_rate"], momentum=0.9, weight_decay=1e-4
        )
    ...
    def train(self, num_epochs, save_name):
        ...
        for epoch in range(num_epochs):
            ...
            print(
                ...
                f"Val F1: {val_f1:.4f}, Recall: {val_recall:.4f}, Prec: {val_precision:.4f}"
            )
...
def run_c3_pruning(prune_rates=[0.2, 0.4, 0.6], corr_penalty_strength=0.3):
    """Run C3 pruning with fine-tuning"""
    ...
    for prune_rate in prune_rates:
        print("\n" + "=" * 60)
        print(f"C3 Pruning with rate: {prune_rate*100:.0f}% (corr_penalty={corr_penalty_strength})")
        print("=" * 60)

        # Compute C3 importance
        print("\n" + "=" * 60)
        print("Step 1: Computing C3 importance (L1 + Activation Correlation)")
        print("=" * 60)
        importance = compute_c3_importance(model, train_loader, device, corr_penalty_strength)

        for name, scores in importance.items():
            print(f"  {name}: {len(scores)} channels")

        # Get keep indices based on C3 importance
        keep_indices = get_keep_indices(importance, prune_rate)

        # Build pruned model
        print("\nStep 2: Building pruned model with C3 channel selection...")
        ...
        print(
            f"  Channel config: c1={channel_config['c1']}, c2={channel_config['c2']}, "
            f"c3={channel_config['c3']}, c4={channel_config['c4']}, c5={channel_config['c5']}"
        )
        ...
        save_name = f"ours_c3_r{int(prune_rate*100)}.pth"
    ...
    for rate in prune_rates:
        save_name = f"ours_c3_r{int(rate*100)}.pth"
        print(f"Ours C3 r{int(rate*100)}: {save_name}")


if __name__ == "__main__":
    set_seed(42)
    run_c3_pruning([0.2, 0.4, 0.6])